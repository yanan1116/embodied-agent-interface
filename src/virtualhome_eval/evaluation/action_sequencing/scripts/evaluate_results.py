...
def evaluate_results(args):
    ...
    error_info = {}
    for output_dict in tqdm(llm_response):
        ...
        try:
            ...
            print(f"Task {task_name}, file {file_id} prediction has format error: {actions}")
            all_parsing_wrong += 1
            actions = None
            format_error = True
        ...
        if actions is None or len(actions) == 0 or not check_name_id_format(actions)[0]:
            all_parsing_wrong += 1
            print(f"Task {task_name}, file {file_id} prediction has no prediction: {actions}")
            format_error = True
