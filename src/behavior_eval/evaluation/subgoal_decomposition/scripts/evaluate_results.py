...
def simulate_one_llm(llm_response_path,  worker_num: int=1, result_dir: str='./results'):
    ...
    eval_stat_path = os.path.join(result_dir, 'log', '.json')
    ...
    #         worker.join()
    # else:
    for demo_name in tqdm(real_task_list):
    ...
    summary['task_num'] = len(real_task_list)
...
def evaluate_results(llm_response_dir, worker_num: int=1, result_dir: str='./results'):
    ...
    simulate_one_llm(llm_response_dir, worker_num, result_dir)
